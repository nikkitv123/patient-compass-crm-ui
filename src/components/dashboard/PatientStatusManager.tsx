      ...
      <Card className="border-healthcare-primary/20 bg-gradient-to-r from-healthcare-light/50 to-white">
        ...
        <CardContent className="space-y-6">
          ...
          <div>
            ...
            <div className="grid gap-3 sm:grid-cols-3">
              {criticalPatients.map(patient => (
                <div key={patient.id} className="bg-white rounded-lg border p-4 shadow-sm hover:shadow-md transition-shadow">
                  <div className="space-y-2">
                    <div className="flex items-start justify-between">
                      <h5 className="font-medium text-sm text-healthcare-dark">{patient.name}</h5>
                      <Badge 
                        className={`text-xs border ${getStatusColor(patient.status)}`}
                        variant="outline"
                      >
                        {patient.status}
                      </Badge>
          ...
          <div className="border-t pt-4">
            <h4 className="text-sm font-medium text-healthcare-dark mb-3">Status Management Actions</h4>
